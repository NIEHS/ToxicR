...
bool PolyKPrepareClass::SetupStudy(std::vector<double> dose,
...
void PolyKPrepareClass::prepare() {
  ...
  std::vector<double> tumorSum(MAX_DOSE_LEVELS,
                               0.0); //* = new double[MAX_DOSE_LEVELS];
  ...
  for (auto &an : m_AnimalList) {
    ...
    if (an.getTumor() == 1) {
      ...
      m_Poly15Denom[doseIdx] =
          m_Poly15Denom[doseIdx] +
          pow(an.getDaysOnStudy() / (double)m_MaxTime, 1.5);
      m_Poly3Denom[doseIdx] = m_Poly3Denom[doseIdx] +
                              pow(an.getDaysOnStudy() / (double)m_MaxTime, 3);
      m_Poly6Denom[doseIdx] = m_Poly6Denom[doseIdx] +
                              pow(an.getDaysOnStudy() / (double)m_MaxTime, 6);
    }
    prevDose = an.getDose();
  }
  ...
  for (int i = 1; i <= m_NumDoseLevels; i++) {
    ...
    //  Rcpp::Rcout << "Poly-3 Denom # " <<  i << " = " << m_Poly3Denom[i]
    //  <<std::endl; Rcpp::Rcout << "TumorSum #" << i << " = " << tumorSum[i] <<
    //  std::endl;
  ...
  for (auto &an : m_AnimalList) {
    ...
    if (an.getTumor() == 1) {
      ...
      m_NonTumorAnimals(ageIdx, doseIdx) =
          m_NonTumorAnimals(ageIdx, doseIdx) + 1;
    }
  }
  m_NAge = ageIdx;
}
