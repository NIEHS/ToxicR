/*
 * $URL:
 * https://svn2.niehs.nih.gov/svn/ntp-cus/dev/tdmse/branches/b_2.5.0.0/src/gov/nih/niehs/tdmse/reporting/pathology/P8_Calculator.java
 ...
 *
 public static void main(String[] args)  {
   ...
   subsetVars.setFilename("C:\\Data\\TDMSE Custom\\PolyK Test\\BCE Male Liver
 HepAd PW4.txt"); subsetVars.setDebugFlag(false);   // Change to true to show
 debug messages subsetVars.prepare(); int ntrt = subsetVars.getNumDoseLevels();
 ...
 *        CF = 0.5 * Max of (nStar[i]/n[i] * (d[i] - dBar) - nStar[i-1]/n[i-1] *
 * [d[i-1] - dBar))
 *
 *  where dBar = Sum (a*d) / Sum(a)
 *        a[i] = nStar[i] * nStar[i] / n[i]
 *        d[i] = dose value at i
 *        nStar[i] = poly-k adjusted sample size (= Sum(t[i,j]**3)  where t =
 * fraction of duration of study) Pi[i] = n[i]/nStar[i] S =
 * sqrt(Sum(Sum((r[i,j]-rBar[i])**2)) / Sum(n[i]-1)) r[i,j] = y[i,j] - piHat *
 * w[i,j] piHat[i] = Sum(y[i,j])/nStar[i]
 *
 *
 * Input variable are:
 *   subsetVars = Input data summarized in terms of number of animals with and
 * without tumors for each day one or more animals were removed ntrt = Number of
 * groups, including control polyKTestType = One of POLY_3_TEST, POLY_6_TEST,
 * POLY_1PT5_TEST cochArmFishPValue = P-value from the Cochran-Armitage test
 * (rarely if ever used)
 */

double TDMSE_PolyK::polyk_mod(PolyKPrepareClass subsetVars, int ntrt,
                              int polyKTestType, double cochArmFishPValue) {
  ...
  try {
    for (int ib = 1; ib <= ntrt; ib++) {
      ai[ib] = 0.0;
      ai2[ib] = 0.0;
      scale[ib] = subsetVars.getWeight(
          ib); /* These are the dose levels scaled from 0 to 1 */
    }
    ...
    for (int dcheck = 1; dcheck <= ntrt; dcheck++) {
      if (subsetVars.getN_subj(dcheck) == 0)
        quitnow = 1;
    }
    ...
    for (int xyz = 1; xyz <= ntrt; xyz++) {
      if (polyKTestType == POLY_3_TEST) {
        ai[xyz] = (subsetVars.getPoly3(xyz) * subsetVars.getPoly3(xyz)) /
                  subsetVars.getN_subj(xyz);
      } else if (polyKTestType == POLY_1PT5_TEST) {
        ai[xyz] = (subsetVars.getPoly15(xyz) * subsetVars.getPoly15(xyz)) /
                  subsetVars.getN_subj(xyz);
      } else if (polyKTestType == POLY_6_TEST) {
        ai[xyz] = (subsetVars.getPoly6(xyz) * subsetVars.getPoly6(xyz)) /
                  subsetVars.getN_subj(xyz);
      }
    ...
    // Calculate CF value
    if (ntrt == 2) // Note this is mathematically the same as the trend
                   // correction if we assume getScale(2) - getScale(1) = 1
    {
      if (polyKTestType == POLY_3_TEST) {
        cf = (ai[1] / (ai[1] + ai[2])) *
      ...
      for (int xyz2 = 2; xyz2 <= ntrt; xyz2++) {
        if (polyKTestType == POLY_3_TEST) {
          front = (subsetVars.getPoly3(xyz2) / subsetVars.getN_subj(xyz2)) *
                  (scale[xyz2] - dBar);
          back =
              (subsetVars.getPoly3(xyz2 - 1) / subsetVars.getN_subj(xyz2 - 1)) *
              (scale[xyz2 - 1] - dBar);
        } else if (polyKTestType == POLY_1PT5_TEST) {
          front = (subsetVars.getPoly15(xyz2) / subsetVars.getN_subj(xyz2)) *
                  (scale[xyz2] - dBar);
          back = (subsetVars.getPoly15(xyz2 - 1) /
                  subsetVars.getN_subj(xyz2 - 1)) *
                 (scale[xyz2 - 1] - dBar);
        } else if (polyKTestType == POLY_6_TEST) {
          front = (subsetVars.getPoly6(xyz2) / subsetVars.getN_subj(xyz2)) *
                  (scale[xyz2] - dBar);
          back =
              (subsetVars.getPoly6(xyz2 - 1) / subsetVars.getN_subj(xyz2 - 1)) *
              (scale[xyz2 - 1] - dBar);
        }
        ai2[xyz2] = front - back;
        if (ai2[xyz2] > cf) {
    ...
    for (int k = 1; k <= ntrt; k++) {
      ...
      /* For each removal age, get the number of animals removed with and
       * without a tumor */
      for (int j = 1; j <= subsetVars.getNage(); j++) {
        xbn = subsetVars.getTumorAnimals(j, k); // Number of tumor animals
        xam =
            subsetVars.getNonTumorAnimals(j, k); // Number of non-tumor animals
        sumj_bn0 += xbn;
        // Set up for summing
    ...
    top =
        sum_adpi - (sum_ad * sum_api / sum_a); // This is A in the big equation
    den_p = sum6 + sum7;
    if (den_p <= 0)
      return pvalue;
    ...
    bot = term3 *
          (sum_add - sum_ad * sum_ad / sum_a); // This is B in the big equation

    // If the bot value is <= 0 we default to the Cochran-Armitage (or Fisher's
    // Exact) p-value
    if (bot <= POLYK_EPSILON)
      return cochArmFishPValue;
    ...
    if (pvalue < MIN_PVALUE)
      pvalue = MIN_PVALUE;
    if (fabs(test_stat) < TESTSTAT_EPSILON)
      pvalue = 0.5;
    if (top < 0)
      pvalue = -1 * pvalue;
...
/**
 * Calculate the chisq probability for the given test statistic and degrees of
 * freedom
 */
double TDMSE_PolyK::chisq(double x2, int idf) {
  ...
  if (fabs(x2) < TESTSTAT_EPSILON && idf == 1)
    return 0.5;
  d2 = idf / 2.0;
  x22 = x2 / 2.0;
  p = 1.0 - gammp(d2, x22);
  ...
  //        factory.createChiSquareDistribution(idf); p = 1 -
  //        chiSq.cummulativeProbability(x2);
