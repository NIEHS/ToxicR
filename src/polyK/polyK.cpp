                                ...
                                double cochArmFishPValue)
  {
    ...
    try
    {
      ...
      if (ntrt == 2) // Note this is mathematically the same as the trend correction if we assume getScale(2) - getScale(1) = 1
      {
        if (polyKTestType == POLY_3_TEST)
        {
          cf = (ai[1] / (ai[1] + ai[2])) * (subsetVars.getPoly3(2) / subsetVars.getN_subj(2)) + (ai[2] / (ai[1] + ai[2])) * (subsetVars.getPoly3(1) / subsetVars.getN_subj(1));
        }
        else if (polyKTestType == POLY_1PT5_TEST)
        {
          cf = (ai[1] / (ai[1] + ai[2])) * (subsetVars.getPoly15(2) / subsetVars.getN_subj(2)) + (ai[2] / (ai[1] + ai[2])) * (subsetVars.getPoly15(1) / subsetVars.getN_subj(1));
        }
        else if (polyKTestType == POLY_6_TEST)
        {
          cf = (ai[1] / (ai[1] + ai[2])) * (subsetVars.getPoly6(2) / subsetVars.getN_subj(2)) + (ai[2] / (ai[1] + ai[2])) * (subsetVars.getPoly6(1) / subsetVars.getN_subj(1));
        }
      }
      else
      {
        ...
        for (int xyz2 = 2; xyz2 <= ntrt; xyz2++)
        {
          if (polyKTestType == POLY_3_TEST)
          {
            front = (subsetVars.getPoly3(xyz2) / subsetVars.getN_subj(xyz2)) * (scale[xyz2] - dBar);
            back = (subsetVars.getPoly3(xyz2 - 1) / subsetVars.getN_subj(xyz2 - 1)) * (scale[xyz2 - 1] - dBar);
          }
          else if (polyKTestType == POLY_1PT5_TEST)
          {
            front = (subsetVars.getPoly15(xyz2) / subsetVars.getN_subj(xyz2)) * (scale[xyz2] - dBar);
            back = (subsetVars.getPoly15(xyz2 - 1) / subsetVars.getN_subj(xyz2 - 1)) * (scale[xyz2 - 1] - dBar);
          }
          else if (polyKTestType == POLY_6_TEST)
          {
            front = (subsetVars.getPoly6(xyz2) / subsetVars.getN_subj(xyz2)) * (scale[xyz2] - dBar);
            back = (subsetVars.getPoly6(xyz2 - 1) / subsetVars.getN_subj(xyz2 - 1)) * (scale[xyz2 - 1] - dBar);
          }
          ai2[xyz2] = front - back;
          if (ai2[xyz2] > cf)
          {
