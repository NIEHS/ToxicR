...
/**
 * Helper class used by P8_Calculator to calculate dose and age vectors for each
 * sex
 *
...
public:
   ...
   * @return Days on dose
   */
  double getT(int idx) { return m_TimeArray[idx]; };
   ...
   */
  int getN_subj(int idx) { return m_NumAnimals[idx]; };

  /**
   * Get the total number of animals across the dose groups
   * @return Total animal count
   */
  int getN_subj_tot() { return m_AnimalList.size(); };
   ...
   */
  double getPoly15(int idx) { return m_Poly15Denom[idx]; };
   ...
   */
  double getPoly3(int idx) { return m_Poly3Denom[idx]; };
   ...
   */
  double getPoly6(int idx) { return m_Poly6Denom[idx]; };
