...
#ifndef lognormal_ILOMAX_aerts_NCH
#define lognormal_ILOMAX_aerts_NCH
...
/*class lognormalILOMAX_aerts_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the Inverse LOMAX model is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalILOMAX_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalILOMAX_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                               int junk)
  ...
  lognormalILOMAX_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::invlomax_aerts; }

  lognormalILOMAX_aerts_BMD_NC(lognormalILOMAX_aerts_BMD_NC &M) {
  ...
  //		double xi = theta(4,0);
  //
  //		Eigen::MatrixXd rV = a*(1.0 + (c - 1.0)*(pow(b / (b + pow(x.array(),
  //-1.0*d)), xi)));
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a * (1.0 + (c - 1.0) * (pow(b / (b + pow(x.array(), -1.0 * d)), xi)));
