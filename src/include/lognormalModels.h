...
public:
  ...
  lognormalLLModel(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                   bool CV = true)
      : lognormalLL(tY, tX, SS){

        };

  virtual cont_model mean_type() { return cont_model::generic; }
  ...
  virtual double bmd_absolute(Eigen::MatrixXd theta, double BMRF,
                              bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                           bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                            bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_point(Eigen::MatrixXd theta, double BMRF,
                           bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_extra(Eigen::MatrixXd theta, double BMRF,
                           bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF,
                                  bool isIncreasing, double BPROB) {
    return 0.0;
  };

  // BASIC BMD Computation absolute to hybrid
  virtual double bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,
                                    double BMRF, bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF,
                                 bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_reldev_bound(Eigen::MatrixXd theta, double BMD,
                                  double BMRF, bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF,
                                 bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF,
                                 bool isIncreasing) {
    return 0.0;
  };
  virtual double bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
                                        double BMRF, bool isIncreasing,
                                        double TAIL_PROB) {
    return 0.0;
  };
  ...
  virtual double bmd_start_extra_hybrid(unsigned n, const double *b,
