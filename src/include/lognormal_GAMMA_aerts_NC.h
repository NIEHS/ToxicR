...
#ifndef lognormal_GAMMA_aerts_NCH
#define lognormal_GAMMA_aerts_NCH
...
/*class lognormalGAMMA_aerts_BMD_NC : public LL
...
class lognormalGAMMA_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalGAMMA_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                              int junk)
  ...
  lognormalGAMMA_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::gamma_aerts; }

  lognormalGAMMA_aerts_BMD_NC(lognormalGAMMA_aerts_BMD_NC &M) {
  ...
  //(c - 1.0)*(gsl_cdf_gamma_P(pow(xx, d), xi, b)));});
  //
  //		// Eigen::MatrixXd rV = a*(1 + (c -
  //1)*(gsl_cdf_gamma_P(pow(x.array(), d), xi, b)));
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx) {
      return a * (1.0 + (c - 1.0) * (gsl_cdf_gamma_P(pow(xx, d), xi, 1.0 / b)));
    });

    // Eigen::MatrixXd rV = a*(1 + (c - 1)*(gsl_cdf_gamma_P(pow(x.array(), d),
    // xi, b)));
