// File:    IDPriorMCMC.h
// Purpose: Sets the priors for the binomial model  and continuous model
// parameters these methods are general
//          and should be able to be used across  classes without much
//          modification
...
//                      basic definition to be used across types of models
...
public:
  ...
  IDPriorMCMC(Eigen::MatrixXd tP)
      : prior_spec(tP){
            ...
            // the correct size matrix and that the bounds make sense in
            // relation
            // to the mean.
        };
  ...
  void scale_prior(double scale, int parm) {
    if (parm >= 0 && parm < prior_spec.rows()) {
      ...
      case prior_iidtype::iid_cauchy:
      case prior_iidtype::iid_gamma:
      case prior_iidtype::iid_pert:
        prior_spec(parm, 1) *= scale;
        prior_spec(parm, 2) *= fabs(
            scale); // note it is only scale because we are dealing with the SD
