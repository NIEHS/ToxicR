...
#ifndef normal_HILL_aerts_NCH
#define normal_HILL_aerts_NCH
...
/*class normalHILL_aerts_BMD_NC : public LL
...
class normalHILL_aerts_BMD_NC : public normalLLModel {
public:
  normalHILL_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                          bool CV, int junk)
  ...
  normalHILL_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::hill_aerts; }

  normalHILL_aerts_BMD_NC(normalHILL_aerts_BMD_NC &M) {
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a *
        (1.0 + (c - 1.0) * (1.0 - pow(b, d) / (pow(b, d) + pow(x.array(), d))));
