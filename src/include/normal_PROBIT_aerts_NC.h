...
#ifndef normal_PROBIT_aerts_NCH
#define normal_PROBIT_aerts_NCH
...
/*class normalPROBIT_aerts_BMD_NC : public LL
...
class normalPROBIT_aerts_BMD_NC : public normalLLModel {
	public:

	normalPROBIT_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,bool SS,
	...
	normalPROBIT_aerts_BMD_NC(){

	};

	virtual cont_model mean_type(){
	  return cont_model::probit_aerts;
	}

	normalPROBIT_aerts_BMD_NC(normalPROBIT_aerts_BMD_NC &M){
	...
	virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta,Eigen::MatrixXd x){
		...
		Eigen::MatrixXd rV = x;
		rV = rV.unaryExpr([&a, &b, &c, &d](double xx){return c*gsl_cdf_ugaussian_P(a + b*pow(xx,d));});
