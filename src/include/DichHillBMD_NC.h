...
// The background and asymptote terms are transformed to a logisitc distribution
// to improve optimization and need to be re-transformed before being used.
...
#define HILL_ADDED_RISK(G, N, A, B, BMR)                                       \
  exp((-A - log((1 - G) * N / BMR - 1.0)) / B)
#define HILL_EXTRA_RISK(G, N, A, B, BMR) exp((-A - log(N / BMR - 1.0)) / B)
// To improve optimization, the model function was intentionally
// re-parameterized such that the background = G, not G*V
...
public:
  ...
  dich_hillModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int T)
      : binomialBMD(tY, tX) {
  ...
  std::vector<double> fixConstraintUB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      temp = (Z - ub * log(BMD)) * 1.1; // add a little extra add a little extra
      ...
      temp = (Z - ub * log(BMD)) *
             1.1; // add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0);
      x[2] = theta(2, 0) - temp;
    }
  ...
  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      Z = HILL_EXTRA_Z(g, n, a, NULL, BMR);
      temp = (lb * log(BMD) - Z) * 2; // add a little extra add a little extra
                                      // so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0);
      x[2] = theta(2, 0) - temp;
      ...
      Z = HILL_ADDED_Z(g, n, a, NULL, BMR);
      temp = (lb * log(BMD) - Z) *
             2; // add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0);
      x[2] = theta(2, 0) - temp;
    }
  ...
  virtual double BMR_CONSTRAINT(Eigen::MatrixXd theta, double *grad, double BMR,
                                double isExtra) {
    double g = HILL_G(theta(0, 0)); // + 0.5;
    double n = HILL_N(theta(1, 0)); // + 0.5;
  ...
  Eigen::MatrixXd beta_BMD_AddedNC(Eigen::MatrixXd theta, double BMD,
                                   double BMR) {
    double g = HILL_G(theta(0, 0));
    double n = HILL_N(theta(1, 0));
    double a = HILL_A(theta(2, 0));
  ...
  Eigen::MatrixXd beta_BMD_ExtraNC(Eigen::MatrixXd theta, double BMD,
                                   double BMR) {
  ...
  double compute_BMD_EXTRA_NC_INEQUALITY(Eigen::MatrixXd theta, double BMD,
                                         double BMR, double inequality,
                                         bool geq, double *grad) {
  ...
  double compute_BMD_ADDED_NC_INEQUALITY(Eigen::MatrixXd theta, double BMD,
                                         double BMR, double inequality,
                                         bool geq, double *grad) {
    double g = HILL_G(theta(0, 0));
    double n = HILL_N(theta(1, 0));
    ...
    if (grad) {
      grad[0] = -(n / (n * g - n + BMR)) *
                (exp(theta(1, 0)) /
                 ((exp(theta(1, 0) + 1) * (exp(theta(1, 0) + 1))))); // gamma
      grad[1] = (g - 1) / ((g - 1) * n + BMR) *
                (exp(theta(1, 0)) /
                 ((exp(theta(1, 0) + 1) * (exp(theta(1, 0) + 1)))));
      grad[2] = -1.0; // alpha
    }
