...
public:
	...
	cont_model modelling_type(){
		  return this->log_likelihood.mean_type();
	}
									   ...
									   double tail_prob)
{
	...
	*/
	if (good_opt)
	{															 // if the opimization criteria worked
...
//	cout << "BMDchange : " << BMDchange << " : " << M->type_of_profile(BMDType) << endl;
...
//			cout << "nans 1 ";
	...
	while (fabs(MAP_LIKE - CLIKE) < totalChange && max_iters < 300 && !error)
	{
		...
		{
			error = true;
//			cout << std::isnan(oR.functionV) << std::isinf(CBMD) << "nan/inf2 ";
		}
