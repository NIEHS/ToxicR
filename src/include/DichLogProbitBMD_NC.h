...
#define LOGPROBIT_ADDED_Z(G, A, BMR)                                           \
  (gsl_cdf_gaussian_Pinv(BMR / (1 - G), 1.0) - A)
...
public:
  ...
  dich_logProbitModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int T)
      : binomialBMD(tY, tX) {
  ...
  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      temp = (lb * log(BMD) - Z) *
             1.2; // add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0) - temp;
    }
    return x;
