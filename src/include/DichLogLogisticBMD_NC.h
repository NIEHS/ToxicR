...
#define LOGLOGISTIC_ADDED_RISK(G, A, B, BMR)                                   \
  exp(LOGLOGISTIC_ADDED_Z(G, A, BMR) / B)
#define LOGLOGISTIC_EXTRA_RISK(G, A, B, BMR)                                   \
  exp(LOGLOGISTIC_EXTRA_Z(G, A, BMR) / B)
...
public:
  ...
  dich_loglogisticModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int T)
      : binomialBMD(tY, tX) {
  ...
  virtual double BMR_CONSTRAINT(Eigen::MatrixXd theta, double *grad, double BMR,
    ...
    if (grad) {
      if (isExtra) {
        ...
        grad[0] =
            -BMR * exp(theta(0, 0)) / pow(BMR + exp(theta(0, 0)) - 1.0, 2.0);
        grad[1] = 0.0;
      }
    }
    return rV;
  ...
  std::vector<double> fixConstraintUB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      Z = LOGLOGISTIC_EXTRA_Z(g, a, BMR);
      temp = (Z - ub * log(BMD)) * 1.01; // add a little extra add a little
                                         // extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0) - temp;
    } else {
      Z = LOGLOGISTIC_ADDED_Z(g, a, BMR);
      temp = (Z - ub * log(BMD)) *
             1.01; // add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0) - temp;
    }
    return x;
  ...
  double compute_BMD_ADDED_NC_INEQUALITY(Eigen::MatrixXd theta, double BMD,
    ...
    if (grad) {
      grad[0] = -exp(theta(0, 0)) /
                ((1.0 + exp(theta(0, 0))) * (exp(theta(0, 0)) + 1.0));
      grad[1] = -1.0; // alpha
    }
