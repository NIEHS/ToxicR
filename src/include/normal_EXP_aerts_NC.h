...
#ifndef normal_EXP_aerts_NCH
#define normal_EXP_aerts_NCH
...
/*class normalEXP_aerts_BMD_NC : public LL
...
class normalEXP_aerts_BMD_NC : public normalLLModel {
public:
  normalEXP_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                         bool CV, int junk)
  ...
  normalEXP_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::exp_aerts; }

  normalEXP_aerts_BMD_NC(normalEXP_aerts_BMD_NC &M) {
  ...
  int nParms() {
    if (isConstVar()) {
      ...
      return 6; // Exponential regression + variance proportional to mean
    };
  }
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a * (1.0 + (c - 1.0) * (1.0 - exp(-b * pow(x.array(), d))));
