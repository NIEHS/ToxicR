...
/*class normalLomax_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the Lomax model is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class normalLomax_BMD_NC : public normalLLModel {
public:
  normalHILL_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, bool CV,
                    int junk)
  ...
  int nParms() {
    if (isConstVar()) {
      return 6; // Lomax regression + constant variance
    } else {
      return 7; // Lomax regression + variance proportional to mean
    };
  }
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd d) {
    ...
    Eigen::MatrixXd rV = gamma + nu * pow(d.array(), n_exp) /
                                     (pow(k, n_exp) + pow(d.array(), n_exp));
