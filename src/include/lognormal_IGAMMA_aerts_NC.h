...
#ifndef lognormal_IGAMMA_aerts_NCH
#define lognormal_IGAMMA_aerts_NCH
...
/*class lognormalIGAMMA_aerts_BMD_NC : public LL
...
class lognormalIGAMMA_aerts_BMD_NC : public lognormalLLModel {
	public:

	lognormalIGAMMA_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,bool SS,
	...
	lognormalIGAMMA_aerts_BMD_NC(){

	};

	virtual cont_model mean_type(){
	  return cont_model::invgamma_aerts;
	}

	lognormalIGAMMA_aerts_BMD_NC(lognormalIGAMMA_aerts_BMD_NC &M){
...
//
//		Eigen::MatrixXd rV = x;
//		rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx){return a*(1.0 + (c - 1.0)*(gsl_cdf_gamma_Q(pow(xx, -1.0*d), xi, b)));});
//
//
//		// Eigen::MatrixXd rV = a*(1 + (c - 1)*(gsl_cdf_gamma_Q(pow(x.array(), -1.0*d), xi, b)));
//
...
//	}
	...
	virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta,Eigen::MatrixXd x){
		...
		rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx){
			...
			return a*(1.0 + (c - 1.0)*(gsl_cdf_gamma_Q(pow(xx, -d), xi, 1.0 / b)));});
