...
#ifndef lognormal_LOGSKEW_aerts_NCH
#define lognormal_LOGSKEW_aerts_NCH
...
/*class lognormalLOGSKEW_aerts_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the log-skew-normal model is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalLOGSKEW_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalLOGSKEW_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                                int junk)
  ...
  lognormalLOGSKEW_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::logskew_aerts; }

  lognormalLOGSKEW_aerts_BMD_NC(lognormalLOGSKEW_aerts_BMD_NC &M) {
  ...
  //(c - 1.0)*(gsl_cdf_ugaussian_P(log(b) + d*log(xx)) - 2.0*tfn(log(b) +
  //d*log(xx), xi)));});
  //
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx) {
      return a * (1.0 + (c - 1.0) * (gsl_cdf_ugaussian_P(log(b) + d * log(xx)) -
