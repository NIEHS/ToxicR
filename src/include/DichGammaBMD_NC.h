...
#define GAMMA_MEAN(G, A, B, D)                                                 \
  ((D) <= (0.0)) ? (G) : (G + (1 - G) * (gsl_cdf_gamma_P(B * D, A, 1)))
...
public:
  static void gsl_err_gamma_extra_z_handler(const char *reason,
                                            ...
                                            int gsl_errno) {
    std::cerr << "GSL error in file " << file << ", line " << line
...
public:
  ...
  dich_gammaModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int Deg)
      : binomialBMD(tY, tX) {
  ...
  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      temp =
          Z / BMD + fabs(lb - Z / BMD) *
                        1.01; // define a beta that is "within" the constraints
      x[0] = theta(0, 0);
