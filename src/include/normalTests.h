...
public:
  normalLLTESTA1(){};
  normalLLTESTA1(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS)
      : normalLL(tY, tX, SS) {
  ...
  //		virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta);
  //		virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd
  //d);

  // VARIANCE FUNCTIONS
  //		Eigen::MatrixXd variance(Eigen::MatrixXd theta);
  //		Eigen::MatrixXd variance(Eigen::MatrixXd theta, Eigen::MatrixXd
  //d);
...
public:
  normalLLTESTA2(){};
  normalLLTESTA2(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS)
      : normalLL(tY, tX, SS) {
    // if it is a sufficient statistics model
    std::vector<double> vec(tX.data(), tX.data() + tX.rows() * tX.cols());
    std::sort(vec.begin(), vec.end());
...
public:
  normalLLTESTA3(){};
  normalLLTESTA3(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS)
      : normalLL(tY, tX, SS) {
    // if it is a sufficient statistics model
    std::vector<double> vec(tX.data(), tX.data() + tX.rows() * tX.cols());
    std::sort(vec.begin(), vec.end());
  ...
  Eigen::MatrixXd variance(Eigen::MatrixXd theta, Eigen::MatrixXd d) {
    ...
    Eigen::MatrixXd rV =
        ...
        pow(abs(tmean.array()),
            theta(theta.rows() - 2, 0)); //  theta(theta.rows() - 2, 0));
    return rV;
  };
