...
#ifndef lognormal_HILL_aerts_NCH
#define lognormal_HILL_aerts_NCH
...
/*class lognormalHILL_aerts_BMD_NC : public LL
...
class lognormalHILL_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalHILL_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                             int junk)
  ...
  lognormalHILL_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::hill_aerts; }

  lognormalHILL_aerts_BMD_NC(lognormalHILL_aerts_BMD_NC &M) {
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a *
        (1 + (c - 1.0) * (1.0 - pow(b, d) / (pow(b, d) + pow(x.array(), d))));
