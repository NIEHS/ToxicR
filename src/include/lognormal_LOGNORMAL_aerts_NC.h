...
#ifndef lognormal_LOGNORMAL_aerts_NCH
#define lognormal_LOGNORMAL_aerts_NCH
...
/*class lognormalLOGNORMAL_aerts_BMD_NC : public LL
...
class lognormalLOGNORMAL_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalLOGNORMAL_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,
                                  bool SS, int junk)
  ...
  lognormalLOGNORMAL_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::lognormal_aerts; }

  lognormalLOGNORMAL_aerts_BMD_NC(lognormalLOGNORMAL_aerts_BMD_NC &M) {
  ...
  //
  //		Eigen::MatrixXd rV = x;
  //		rV = rV.unaryExpr([&a, &b, &c, &d](double xx){return a*(1.0 + (c
  //- 1.0)*gsl_cdf_ugaussian_P(log(b) + d*log(xx)));});
  //
  //
