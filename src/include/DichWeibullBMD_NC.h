...
#define WEIBULL_ADDED_RISK(G, A, B, BMR)                                       \
  WEIBULL_ADDED_Z(G, A, BMR) / pow(B, 1.0 / A)
#define WEIBULL_EXTRA_RISK(G, A, B, BMR)                                       \
  WEIBULL_EXTRA_Z(G, A, BMR) / pow(B, 1.0 / A)
#define WEIBULL_MEAN(G, A, B, D)                                               \
  ((D) <= (0.0)) ? (G) : (G + (1 - G) * (1 - exp(-B * pow(D, A))))
...
public:
  ...
  dich_weibullModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int Deg)
      : binomialBMD(tY, tX) {
  ...
  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      // lco: Added the line below to resolve using "temp" before it is
      // initialized. It was not in Matt's latest code as of 7/2/2018.
      temp = Z / pow(BMD, a); // temp is the 'new beta with constraint satisfied
      if (temp < lb) {
        temp = Z / pow(BMD, a) + fabs(Z / pow(BMD, a) - lb) * 1.2;
        x[0] = theta(0, 0);
