...
#ifndef lognormal_ILOGSKEW_aerts_NCH
#define lognormal_ILOGSKEW_aerts_NCH
...
/*class lognormalILOGSKEW_aerts_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the inverse log-skew-normal model is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalILOGSKEW_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalILOGSKEW_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,
                                 bool SS, int junk)
  ...
  lognormalILOGSKEW_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,
                                 bool SS, bool CV, int junk)
      : lognormalLLModel(tY, tX, SS){
            // if it is a sufficient statistics model
        };

  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta) { return mean(theta, X); }
  lognormalILOGSKEW_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::invlogskew_aerts; }

  lognormalILOGSKEW_aerts_BMD_NC(lognormalILOGSKEW_aerts_BMD_NC &M) {
  ...
  //
  //		Eigen::MatrixXd rV = x;
  //		rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx){return a*(1.0 +
  //(c - 1.0)*(1.0 - gsl_cdf_ugaussian_P(log(b) - d*log(xx)) + 2.0*tfn(log(b) -
  //d*log(xx), xi)));});
  //
  //
  //		// Eigen::MatrixXd rV = a*(1 + (c - 1)*gsl_cdf_ugaussian_P(log(b) +
  //d*log(x.array())));
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx) {
      return a * (1.0 +
                  (c - 1.0) * (1.0 - gsl_cdf_ugaussian_P(log(b) - d * log(xx)) +
                               2.0 * tfn(log(b) - d * log(xx), xi)));
    });
