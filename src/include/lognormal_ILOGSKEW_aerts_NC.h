...
class lognormalILOGSKEW_aerts_BMD_NC : public lognormalLLModel {
	...
	lognormalILOGSKEW_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,bool SS,
	...
	virtual cont_model mean_type(){
	  return cont_model::invlogskew_aerts;
	}
...
//
//		Eigen::MatrixXd rV = x;
//		rV = rV.unaryExpr([&a, &b, &c, &d, &xi](double xx){return a*(1.0 + (c - 1.0)*(1.0 - gsl_cdf_ugaussian_P(log(b) - d*log(xx)) + 2.0*tfn(log(b) - d*log(xx), xi)));});
//
//
