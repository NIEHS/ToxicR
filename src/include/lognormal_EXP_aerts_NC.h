...
#ifndef lognormal_EXP_aerts_NCH
#define lognormal_EXP_aerts_NCH
...
/*class lognormalEXP_aerts_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the EXP model (Aerts version) is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalEXP_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalEXP_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS,
                            int junk)
  ...
  lognormalEXP_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::exp_aerts; }

  lognormalEXP_aerts_BMD_NC(lognormalEXP_aerts_BMD_NC &M) {
  ...
  int nParms() {
    ...
    return 5; // Exponential regression + constant variance
  }
  ...
  //		double d = theta(3,0);
  //
  //		Eigen::MatrixXd rV = a*(1.0 + (c - 1.0)*(1.0 - exp(-1.0*b *
  //pow(x.array(), d))));
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a * (1.0 + (c - 1.0) * (1.0 - exp(-1.0 * b * pow(x.array(), d))));
