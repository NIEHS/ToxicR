...
template <class LL, class PR> class dBMDModel : public statModel<LL, PR> {
public:
  dBMDModel(LL t_L, PR t_PR, std::vector<bool> b_fixed,
            std::vector<double> d_fixed)
      : statModel<LL, PR>(t_L, t_PR, b_fixed, d_fixed){};

  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
                                      double BMR, bool isExtra, double lb) {
    ...
    return statModel<LL, PR>::log_likelihood.fixConstraintLB(theta, BMD, BMR,
                                                             isExtra, lb);
  }

  std::vector<double> fixConstraintUB(Eigen::MatrixXd theta, double BMD,
                                      double BMR, bool isExtra, double ub) {
    ...
    return statModel<LL, PR>::log_likelihood.fixConstraintUB(theta, BMD, BMR,
                                                             isExtra, ub);
  }

  double BMR_CONSTRAINT(Eigen::MatrixXd theta, double *grad, double BMR,
                        bool isExtra) {
    ...
    return statModel<LL, PR>::log_likelihood.BMR_CONSTRAINT(theta, grad, BMR,
                                                            isExtra);
  }
  ...
  double added_riskBMDNC(double BMR) {
    ...
    return statModel<LL, PR>::log_likelihood.compute_BMD_ADDED_NC(
        statModel<LL, PR>::theta, BMR);
  };

  double extra_riskBMDNC(double BMR) {
    // to do check if the model has an estimate
    // set all the fixed parameters to their value
    return statModel<LL, PR>::log_likelihood.compute_BMD_EXTRA_NC(
        this->getEST(), BMR);
  };

  Eigen::MatrixXd start_equalityOptim(Eigen::MatrixXd theta, double BMD,
                                      double BMR, bool isExtra) {
    ...
    if (isExtra) {
      return statModel<LL, PR>::log_likelihood.beta_BMD_ExtraNC(theta, BMD,
                                                                BMR);
    } else {
      return statModel<LL, PR>::log_likelihood.beta_BMD_AddedNC(theta, BMD,
                                                                BMR);
    }
  }

  Eigen::MatrixXd bmdAddedNC_equals(Eigen::MatrixXd theta, double BMD,
                                    double BMR) {
    return statModel<LL, PR>::log_likelihood.beta_BMD_AddedNC(theta, BMD, BMR);
  }

  Eigen::MatrixXd bmdExtraNC_equals(Eigen::MatrixXd theta, double BMD,
                                    double BMR) {
  ...
  double equality_extra(Eigen::MatrixXd theta, double *grad, double BMD,
                        double BMR) {
    // set all the fixed parameters to their value
    for (size_t i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
    ...
    return statModel<LL, PR>::log_likelihood.compute_BMD_EXTRA_NC_EQUALITY(
        theta, grad, BMD, BMR);
  }
  double equality_added(Eigen::MatrixXd theta, double *grad, double BMD,
                        double BMR) {
    // set all the fixed parameters to their value
    for (size_t i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
    ...
    return statModel<LL, PR>::log_likelihood.compute_BMD_ADDED_NC_EQUALITY(
        theta, grad, BMD, BMR);
  }

  virtual double inequality_extra(Eigen::MatrixXd theta, double BMD, double BMR,
                                  double inequality, bool geq, double *grad) {
    // set all the fixed parameters to their value
    for (size_t i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
        theta(i, 0) = this->fixedV[i];
  ...
  virtual double inequality_added(Eigen::MatrixXd theta, double BMD, double BMR,
                                  double inequality, bool geq, double *grad) {
    // set all the fixed parameters to their value
    for (size_t i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
...
template <class A, class B> struct optimInfo {
  dBMDModel<A, B> *sm;
  double cBMD;
  double BMR;
  bool isExtra;
...
template <class A, class B> struct inequalityInfo : public optimInfo<A, B> {
...
template <class LL, class PR>
double inequality_constraint(unsigned n, const double *b, double *grad,
                             void *data) {
  ...
  if (model->isExtra) {
    return model->sm->inequality_extra(theta, model->cBMD, model->BMR,
                                       model->inequality, model->geq, grad);
  } else {
    return model->sm->inequality_added(theta, model->cBMD, model->BMR,
                                       model->inequality, model->geq, grad);
  }
}
...
//         function of a vector of k parameters).
...
template <class LL, class PR>
double inequality_constraint_general(unsigned n, const double *b, double *grad,
                                     void *data) {
  inequalityInfo<LL, PR> *model = (inequalityInfo<LL, PR> *)data;
  Eigen::MatrixXd theta(n, 1);
  for (unsigned int i = 0; i < n; i++)
...
findMAX_W_EQUALITY(dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
  ...
  // which makes this code not generalizable to models requiring equality
  // constraints
  ...
  if (!lbsatisfied || !ubsatisfied) {
    ...
    if (isExtra) {
      ...
      x[x.size() - 1] =
          ((-log(1.0 - BMR / (1 - g)) - t_sum) / pow(BMD, x.size() - 1));
    }
  }
  ...
  nlopt::opt local_opt(nlopt::LD_LBFGS,
                       M->nParms()); // set up the local optimizer
  ...
  // file << __FUNCTION__ << " at line: " << __LINE__ << " Before optimize"<<
  // endl;
...
template <class LL, class PR>
optimizationResult
findMAX(dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
        const bool isExtra, // true if it is false if it is added
        double BMD, double BMR, const int iter, nlopt::algorithm algorithm) {
  optimizationResult oR;
  if (!M->fixedNC_BMDformula()) {
    // there is no way to express one of the parameters as a function of the
    // others thus we have to do this using equality constraints
  ...
  } else { // our starting values are assumed to have the extra parameter
           // removed
  ...
  if (isExtra) {
    ...
    t2 =
        M->inequality_extra(temp, BMD, BMR, ineqinfoUB.inequality, false, NULL);
  } else {
    t1 = M->inequality_added(temp, BMD, BMR, ineqinfoLB.inequality, true, NULL);
    t2 =
        M->inequality_added(temp, BMD, BMR, ineqinfoUB.inequality, false, NULL);
  }
  // for (double g:x){ cout << g << endl; }
  ...
  opt.add_inequality_constraint(inequality_constraint_general<LL, PR>,
                                &ineqinfoUB);
...
fit_profileLogic(dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
                 const bool isExtra, // true if it is false if it is added
                 double BMD, double BMR, const int iter,
                 nlopt::algorithm algorithm) {
  optimizationResult oR;
  std::list<Eigen::MatrixXd> rV;
  Eigen::MatrixXd result(3, 1);
  Eigen::MatrixXd mParms,
      cParms; // mParms - Maximized parms in the restricted model
...
template <class LL, class PR>
Eigen::MatrixXd
profile_BMDNC(dBMDModel<LL, PR> *M,
              const bool isExtra, // true if it is false if it is added
              const double BMD, const double BMR, const double BMDchange,
              const double totalChange, bool robust)

{
  ...
  while (fabs(MAP_LIKE - CLIKE) < totalChange && max_iters < 500 &&
         CBMD > 1e-8) {

    // fit the first profile likelihood option
    temp1 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 10000,
                                     nlopt::LN_COBYLA);
    ...
    // second opinion
    if (true) {
      temp2 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 250,
                                       nlopt::LD_MMA);
      ret1 = temp1.front();
      ret2 = temp2.front();
  ...
  while (max_iters2 < 200 && CBMD < 2.5 * max_dose &&
    ...
    // second opinion
    if (robust) {
      temp2 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 150,
                                       nlopt::LD_MMA);
