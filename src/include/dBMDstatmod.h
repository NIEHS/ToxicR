						   ...
						   nlopt::algorithm algorithm)
{
	...
	if (start.rows() == M->nParms())
	{ // we need to remove the extra parameter
		int j = 0;
		for (int i = 0; i < M->nParms(); i++)
		{
			if (i != rp)
	...
	{ // our starting values are assumed to have the extra parameter removed
...

{
	...
	while (fabs(MAP_LIKE - CLIKE) < totalChange && max_iters < 500 && CBMD > 1e-8)
	{
		...
		if (true)
		{
			...
			{ // first optimization was the best
		...
		max_iters++;
		if (error)
		{ // there was an error in the optimization we break
			break;
		}
	}
