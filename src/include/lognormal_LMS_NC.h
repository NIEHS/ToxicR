...
#ifndef lognormal_LMS_NCH
#define lognormal_LMS_NCH
...
/*class lognormalLMS_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the HILL model (Aerts version) is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalLMS_BMD_NC : public lognormalLLModel {
public:
  lognormalLMS_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, int junk)
      : lognormalLLModel(tY, tX, SS){
            // if it is a sufficient statistics model
        };

  lognormalLMS_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, bool CV,
                      int junk)
      : lognormalLLModel(tY, tX, SS){
            // if it is a sufficient statistics model
        };
  ...
  virtual cont_model mean_type() { return cont_model::hill_aerts; }

  lognormalLMS_BMD_NC(lognormalLMS_BMD_NC &M) {
  ...
  //		double d = theta(3,0);
  //
  //		Eigen::MatrixXd rV = a*(1 + (c - 1.0)*(1.0 - b / (b + pow(x.array(),
  //d))));
  //
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV =
        a *
        (1.0 + (c - 1.0) * (1.0 - exp(-b * x.array() - d * pow(x.array(), 2))));
