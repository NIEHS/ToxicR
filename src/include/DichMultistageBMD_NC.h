...
public:
  ...
  dich_multistageNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int T)
      : binomialBMD(tY, tX) {
    degree = T;
  ...
  bool fixedNC_BMDformula() {
    return false;
  } // for the multistage this is not true

  // Computes the EQUALITY BMD with a given BMR assuming no covariates NC option
  virtual double compute_BMD_ADDED_NC_EQUALITY(Eigen::MatrixXd theta,
                                               double *grad, double BMD,
                                               double BMR) {
    double g = MULTISTAGE_G(theta(0, 0));
  ...
  virtual double compute_BMD_EXTRA_NC_EQUALITY(Eigen::MatrixXd theta,
                                               double *grad, double BMD,
                                               double BMR) {
    // double g = MULTISTAGE_G(theta(0, 0));
    Eigen::MatrixXd BETA = MULTISTAGE_B(theta, degree);
