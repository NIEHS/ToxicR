...
#define LOGISTIC_ADDED_Z(A, BMR)                                               \
  -log((1 - ADDED_Q(A, BMR)) / (1 + ADDED_Q(A, BMR) * exp(-A)))
#define LOGISTIC_EXTRA_Z(A, BMR)                                               \
  -log((1 - EXTRA_Q(A, BMR)) / (1 + EXTRA_Q(A, BMR) * exp(-A)))
...
public:
  ...
  dich_logisticModelNC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, int T)
      : binomialBMD(tY, tX) {
  ...
  std::vector<double> fixConstraintLB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      // temp = (lb*log(BMD) - Z) *1.2; // add a little extra add a little extra
      // so the constraint is satisfied
      x[0] = theta(0, 0);
    } else {
      // Z = LOGISTIC_ADDED_Z(a, BMR);
      // temp = (lb*log(BMD/) - Z) * 1.2; // add a little extra so the
  ...
  std::vector<double> fixConstraintUB(Eigen::MatrixXd theta, double BMD,
    ...
    if (isExtra) {
      ...
      // Z = LOGISTIC_ADDED_Z(a, BMR);
      // temp = (Z - ub * log(BMD))*1.01; // add a little extra so the
      // constraint is satisfied
      x[0] = theta(0, 0);
      // x[1] = theta(1, 0) - temp;
    }
