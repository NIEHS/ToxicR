...
#ifndef lognormal_LOGISTIC_aerts_NCH
#define lognormal_LOGISTIC_aerts_NCH
...
/*class lognormalLOGISTIC_aerts_BMD_NC : public LL
 * This class defines a normal log-likelihood where
 * the LOGISTIC model is the model that is fit to the data.
 * NC stands for no covariates, for future extentions that
 */

/*implements the basic model model Y = X*/

class lognormalLOGISTIC_aerts_BMD_NC : public lognormalLLModel {
public:
  lognormalLOGISTIC_aerts_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX,
                                 bool SS, int junk)
  ...
  lognormalLOGISTIC_aerts_BMD_NC(){

  };

  virtual cont_model mean_type() { return cont_model::logistic_aerts; }

  lognormalLOGISTIC_aerts_BMD_NC(lognormalLOGISTIC_aerts_BMD_NC &M) {
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd x) {
    ...
    Eigen::MatrixXd rV = c / (1.0 + exp(-1.0 * a - b * pow(x.array(), d)));
  ...
  //		double d = theta(3,0);
  //
  //		Eigen::MatrixXd rV = c / (1.0 + exp(-1.0*a - b*pow(x.array(),
  //d)));
  //
  //		return log(rV.array());
  //	}
