...
public:
  normalFUNL_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, bool CV,
                    int junk)
  ...
  virtual cont_model mean_type() { return cont_model::funl; }
  ...
  double FUNL_mean(double d, Eigen::MatrixXd A) {
    ...
    double rV =
        A(0, 0) + A(1, 0) * exp(-exp(A(5, 0)) * (d - A(4, 0)) * (d - A(4, 0))) /
                      (1 + exp(-(1 / A(3, 0)) * (d - A(2, 0))));
    return rV;
  }

  double dFUNL_mean(double d, Eigen::MatrixXd A) {
    double rV = -2 * exp(A(5, 0)) * (d - A(4, 0)) +
                (1 / A(3, 0)) * exp(-(1 / A(3, 0)) * (d - A(2, 0))) /
                    (1 + exp(-(1 / A(3, 0)) * (d - A(2, 0))));
    return rV;
  }
  ...
  double d2FUNL_mean(double d, Eigen::MatrixXd A) {
    double temp = (1 / A(3, 0));
    temp *= temp;
    double temp2 = ((1 + exp(-(1 / A(3, 0)) * (d - A(2, 0)))));
    temp2 *= temp2;

    double rV =
        -2 * exp(A(5, 0)) - (temp)*exp(-(1 / A(3, 0)) * (d - A(2, 0))) / temp2;
    return rV;
  }
