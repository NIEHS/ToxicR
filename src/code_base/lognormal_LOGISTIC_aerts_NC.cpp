...
//////////////////////////////////////////////////////////////////////////
int lognormalLOGISTIC_aerts_BMD_NC::type_of_profile(contbmd TYPE)
{
...
//                       An equality constraint is used here.
//////////////////////////////////////////////////////////////////////////
int lognormalLOGISTIC_aerts_BMD_NC::parameter_to_remove(contbmd TYPE) {
...
*           can be performed. This function is used in an optimization that finds the
			...
			unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_absolute(unsigned n,
											  const double *b,
											  double *grad,
											  void   *data){
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_absolute_clean(std::vector<double> x,
													...
													bool isIncreasing)
{
	...
	// double f0 = x[2] / (1.0 + exp(-1.0 * x[0]));
	// // temp = (BMRF) / fabs(temp - 1.0 / (1.0 + exp(-1.0 * x[0])));
	// temp = (log(BMRF + exp(f0)))/ temp;
	// x[2] = temp;
	double temp = BMRF + exp(x[2] / (1.0 + exp(-x[0])));
	temp = log(temp) / x[2];
...
unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_reldev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_reldev_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double t = 0.0;
	...
	double temp = t * exp(x[2] / (1.0 + exp(-x[0])));
	temp = log(temp) / x[2];
	temp = -1.0 * log(1.0 / temp - 1.0);
	// temp = (x[0] - temp) / pow(BMD, x[3]);
	temp = (temp - x[0]) / pow(BMD, x[3]);
...
unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_stddev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_stddev_clean(std::vector<double> x,
															  double BMRF, double BMD,
															  bool isIncreasing)
{
	if (!isIncreasing)
...
unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_point(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_point_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double temp = 0;
...
unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_extra(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
																bool isIncreasing)
{
...
unbounded - nu
*/
double  lognormalLOGISTIC_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
												  const double *b,
												  double *grad,
												  void   *data) {
...
std::vector<double> lognormalLOGISTIC_aerts_BMD_NC::bmd_start_hybrid_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
...
// Functions: lognormalLOGISTIC_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalLOGISTIC_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalLOGISTIC_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//            lognormalLOGISTIC_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note they are  call the code
//            in  lognormalLOGISTIC_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalLOGISTIC_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(2,1); d << 0.0, BMD;
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalLOGISTIC_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,,double BPROB)
...
//
////////////////////////////////////////////////////////////////////////
double lognormalLOGISTIC_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
...
//            in  lognormalLOGISTIC_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalLOGISTIC_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = variance(theta,d);
	double t = temp(0,0);
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	// note isIncreasing is ignored as point is specified by the user
...
double lognormalLOGISTIC_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	double nu = theta(1,0);
	temp = exp(temp.array());
...
//
////////////////////////////////////////////////////////////////////////
double lognormalLOGISTIC_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,double TAIL_PROB){
...
// double lognormalLOGISTIC_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd theta)
// {
