...
//  as a function
//						 of the fixed BMD. The optimizer
//thus optimizes a smaller problem
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_start_absolute(unsigned n,
  ...
  //	temp = 1.0 - 2.0 * gsl_ran_ugaussian_pdf(temp) *
  // gsl_cdf_ugaussian_P(b[4]*temp);
...
std::vector<double> lognormalILOGSKEW_aerts_BMD_NC::bmd_start_absolute_clean(
...
std::vector<double> lognormalILOGSKEW_aerts_BMD_NC::bmd_start_reldev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double t = 0.0;
  ...
  temp = 1.0 - gsl_cdf_ugaussian_P(temp) + 2.0 * tfn(temp, x[4]);
  //	temp = 1.0 - 2.0 * gsl_ran_ugaussian_pdf(temp) *
  // gsl_cdf_ugaussian_P(x[4]*temp); 	temp = (x[2] - 1.0) * temp * (1.0 / t);
  // temp = 1.0 + t / temp;
...
std::vector<double> lognormalILOGSKEW_aerts_BMD_NC::bmd_start_stddev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  if (!isIncreasing)
    BMRF *= 1;
...
std::vector<double> lognormalILOGSKEW_aerts_BMD_NC::bmd_start_point_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double temp = log(x[1]) - x[3] * log(BMD);
  temp = 1.0 - gsl_cdf_ugaussian_P(temp) + 2.0 * tfn(temp, x[4]);
  //	temp = 1.0 - 2.0 * gsl_ran_ugaussian_pdf(temp) *
...
std::vector<double> lognormalILOGSKEW_aerts_BMD_NC::bmd_start_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
  ...
  //     sdata->isIncreasing ? NOT_ADVERSE_P : TAIL_PROB, mu_zero, std_zero);
  /////////////////////////////////////////////////////////////////////////////////////
  double returnV = 0.0;
  double temp;
...
std::vector<double>
lognormalILOGSKEW_aerts_BMD_NC::bmd_start_hybrid_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing,
    double tail_prob) {
...
// Functions: lognormalILOGSKEW_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd
// theta, double BMRF, bool isIncreasing)
//			  lognormalILOGSKEW_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd
// theta, double BMRF, bool isIncreasing)
//			  lognormalILOGSKEW_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd
// theta, double BMRF, bool isIncreasing)
//            lognormalILOGSKEW_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta,
...
//            in  lognormalILOGSKEW_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd
//            theta, double BMRF, bool isIncreasing)
//
double lognormalILOGSKEW_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta,
                                                          double BMD,
                                                          double BMRF,
                                                          bool isIncreasing) {
  Eigen::MatrixXd d(2, 1);
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta,
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta,
                                                        double BMD, double BMRF,
                                                        bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta,
                                                       double BMD, double BMRF,
                                                       bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta,
                                                       double BMD, double BMRF,
                                                       bool isIncreasing) {
...
////////////////////////////////////////////////////////////////////////
// Function:  double
// lognormalILOGSKEW_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
// double BMRF, bool isIncreasing,,double BPROB) Variables: theta - matrix of
// theta values for the model
//			  BMRF  - This is a value between 0 and 1 that describes
//the increased probability over BPROB
...
//
////////////////////////////////////////////////////////////////////////
double lognormalILOGSKEW_aerts_BMD_NC::bmd_hybrid_extra_bound(
...
//            theta, double BMRF, bool isIncreasing)
//
double lognormalILOGSKEW_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
                                                    double BMRF,
                                                    bool isIncreasing) {
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta,
                                                 double BMRF,
                                                 bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
// double lognormalILOGSKEW_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta,
// double BMRF, bool isIncreasing){ 	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
//	Eigen::MatrixXd temp = mean(theta,d);
//
//	double t = 0.0;
//
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta,
                                                 double BMRF,
                                                 bool isIncreasing) {
  // note isIncreasing is ignored as point is specified by the user
...
double lognormalILOGSKEW_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta,
                                                 double BMRF,
                                                 bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
//
////////////////////////////////////////////////////////////////////////
double lognormalILOGSKEW_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
                                                        double BMRF,
                                                        bool isIncreasing,
                                                        double TAIL_PROB) {
...
// double lognormalILOGSKEW_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd
// theta)
// {
...
// * log(1 / sqrt(2.0 * M_PI)) - (Y.col(2).array() / 2.0) * log(var.array()) -
// (1 / (2.0 * var.array())) * ((Y.col(2).array() - 1) * pow(Y.col(1).array(),
// 2) + Y.col(2).array() * pow(Y.col(0).array() - mu.array(), 2));
// 	}
// 	else
// 	{
// 		// log normal likelihood
// 		Eigen::MatrixXd sqerr = pow(log(Y.col(0).array()) - mu.array(),
// 2); 		returnV = -log(Y.col(0).array()) - (0.5) * log(2 * M_PI *
// var.array()) - (1 / (2.0 * var.array())) * sqerr.array();
// 	}
