...
Eigen::MatrixXd convertresult_to_probs(Eigen::MatrixXd data) {
  ...
  for (int i = 0; i < result.rows(); i++) {
    ...
    result(i, 0) = result(i, 0) < 0
                       ? 1e4
                       : result(i, 0); // numerical zero for calculations
    result(i, 0) = is_afterMax
                       ? .5 + gsl_cdf_chisq_P(2.0 * result(i, 0), 1.0) / 2.0
                       : .5 - gsl_cdf_chisq_P(2.0 * result(i, 0), 1.0) / 2.0;
  ...
  for (int i = 1; i < result.rows(); i++) {
    if (result(i, 2) <= 0.0) {
      ...
      if (i != result.rows() - 1) { // if it is not the last one we have
        result(i + 1, 2) =
            result(i + 1, 0) - result(i, 0); // we must update the difference
      }
    }
  }
...
double find_maBMDquantile(double q, std::vector<double> probs,
  ...
  while ((fabs(log(temp / q)) > 1e-8) && !std::isinf(temp) &&
         !std::isnan(temp)) { // use the log ratio in the calculation as it is
                              // invariant to the size of q
    if (temp > q) {
      max = test;
    } else {
      min = test;
