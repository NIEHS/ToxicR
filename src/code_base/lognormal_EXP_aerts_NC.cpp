...
int lognormalEXP_aerts_BMD_NC::type_of_profile(contbmd TYPE)
{
...
//////////////////////////////////////////////////////////////////////////
int lognormalEXP_aerts_BMD_NC::parameter_to_remove(contbmd TYPE) {
...
*           can be performed. This function is used in an optimization that finds the
			...
			unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_absolute(unsigned n,
											  const double *b,
											  double *grad,
											  void   *data){
	...
	temp = 1.0 - exp(-1.0 * b[1] * pow(sdata->BMD, b[3]));
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_absolute_clean(std::vector<double> x,
													...
													bool isIncreasing)
{
	...
	double temp = 0.0;
	temp = 1.0 - exp(-1.0 * x[1] * pow(BMD, x[3]));
...
unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_reldev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_reldev_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double t = 0.0;
...
unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_stddev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_stddev_clean(std::vector<double> x,
															  double BMRF, double BMD,
															  bool isIncreasing)
{
	if (!isIncreasing)
...
unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_point(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_point_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double temp = 0;
...
unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_extra(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
																bool isIncreasing)
{
...
unbounded - nu
*/
double  lognormalEXP_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
												  const double *b,
												  double *grad,
												  void   *data) {
...
std::vector<double> lognormalEXP_aerts_BMD_NC::bmd_start_hybrid_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
...
// Functions: lognormalEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalEXP_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalEXP_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//            lognormalEXP_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note they are  call the code
//            in  lognormalEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalEXP_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(2,1); d << 0.0, BMD;
...
double lognormalEXP_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
double lognormalEXP_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalEXP_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalEXP_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalEXP_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,,double BPROB)
...
//
////////////////////////////////////////////////////////////////////////
double lognormalEXP_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
...
//            in  lognormalEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	...
	double temp = -1.0 * log(1.0 - q);
	temp = pow(temp / b, 1.0/d);

	return temp;

}
double lognormalEXP_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = variance(theta,d);
...
double lognormalEXP_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalEXP_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	// note isIncreasing is ignored as point is specified by the user
...
double lognormalEXP_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	double nu = theta(1,0);
...
//
////////////////////////////////////////////////////////////////////////
double lognormalEXP_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,double TAIL_PROB){
...
// double lognormalEXP_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd theta)
// {
