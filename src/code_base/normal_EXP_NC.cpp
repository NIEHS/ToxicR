...
*/
double normalEXPONENTIAL_BMD_NC::bmd_start_absolute(unsigned n, const double *b,
...
std::vector<double> normalEXPONENTIAL_BMD_NC::bmd_start_absolute_clean(
  ...
  if (!isIncreasing)
    BMRF *= 1.0;
  x[0] = 1.0; /// necessary to initialize x[0] to something otherwise it is nan
              /// and it blows up
...
essentially unbounded - nu
*************************************************************************************************/
double normalEXPONENTIAL_BMD_NC::bmd_start_reldev(unsigned n, const double *b,
                                                  double *grad, void *data) {
  ...
  case NORMAL_EXP3_UP:
    ...
    temp = sdata->isIncreasing ? pow(log(t + 1.0), 1 / b[3])
                               : pow(-log(1.0 - t), 1 / b[3]);
    temp = temp / sdata->BMD;
    break;
  ...
  case NORMAL_EXP4_UP:
    ...
    temp = sdata->isIncreasing
               ? log(-1.0 * (t + 1.0 - exp(b[2])) / (b[2] - 1.0))
  ...
  default:
    ...
    temp = sdata->isIncreasing
               ? log(-1.0 * (t + 1.0 - exp(b[2])) / (exp(b[2]) - 1.0))
               : log((t - 1.0 + exp(b[2])) / (exp(b[2]) - 1.0));
  ...
  double returnV = 0.0;
  for (unsigned int i = 0; i < n; i++)
    returnV += pow(theta_2(i, 0) - theta(i, 0), 2.0);
...
std::vector<double> normalEXPONENTIAL_BMD_NC::bmd_start_reldev_clean(
  ...
  case NORMAL_EXP3_UP:
    // case 3 we are dealing with the term 2
    temp = isIncreasing ? pow(log(t + 1.0), 1 / x[3])
                        : pow(-log(1.0 - t), 1 / x[3]);
    temp = temp / BMD;
    break;
  case NORMAL_EXP4_DOWN:
...
essentially unbounded - nu
*/
double normalEXPONENTIAL_BMD_NC::bmd_start_stddev(unsigned n, const double *b,
                                                  double *grad, void *data) {
...
essentially unbounded - nu
*/
double normalEXPONENTIAL_BMD_NC::bmd_start_point(unsigned n, const double *b,
                                                 double *grad, void *data) {
...
essentially unbounded - nu
*/
double normalEXPONENTIAL_BMD_NC::bmd_start_extra(unsigned n, const double *b,
                                                 double *grad, void *data) {
...
double normalEXPONENTIAL_BMD_NC::bmd_start_hybrid_extra(unsigned n,
  ...
  Eigen::MatrixXd mu = mean(
      thetas_d,
      d); // compute the mean at background an BMD
          //////////////////////////////////////////////////////////////////////
  ...
  if (sdata->isIncreasing) {
    ...
    if (!isConstVar())
      temp = pow(mu(1, 0), b[deg + 1] / 2.0) * k_1 -
             pow(mu(0, 0), b[deg + 1] / 2.0) * k_0;
    else
      temp = k_1 - k_0;
    ...
    if (!isConstVar())
      temp = pow(mu(0, 0), b[deg + 1] / 2.0) * k_0 -
             pow(mu(1, 0), b[deg + 1] / 2.0) * k_1;
    else
      temp = k_0 - k_1;
...
std::vector<double> normalEXPONENTIAL_BMD_NC::bmd_start_hybrid_extra_clean(
  ...
  if (isIncreasing) {
    ...
    if ((int)x.size() == deg + 3)
      temp = pow(mu(1, 0), x[deg + 1] / 2.0) * k_1 -
             pow(mu(0, 0), x[deg + 1] / 2.0) * k_0;
    else
      temp = k_1 - k_0;
    ...
    if (!isConstVar())
      temp = pow(mu(0, 0), x[deg + 1] / 2.0) * k_0 -
             pow(mu(1, 0), x[deg + 1] / 2.0) * k_1;
    else
      temp = k_0 - k_1;
...
// Functions: normalEXPONENTIAL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
// double BMRF, bool isIncreasing)
//			  normalEXPONENTIAL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta,
//double BMRF, bool isIncreasing)
//			  normalEXPONENTIAL_BMD_NC::bmd_reldev(Eigen::MatrixXd
//theta, double BMRF, bool isIncreasing)
//            normalEXPONENTIAL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double
//            BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note
// they are  call the code
//            in  normalEXPONENTIAL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
//            double BMRF, bool isIncreasing)
//
double normalEXPONENTIAL_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta,
...
double normalEXPONENTIAL_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta,
                                                 double BMD, double BMRF,
                                                 bool isIncreasing) {
  Eigen::MatrixXd d(2, 1);
  d << 0.0, BMD;
...
double normalEXPONENTIAL_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalEXPONENTIAL_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta,
                                                 double BMD, double BMRF,
                                                 bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalEXPONENTIAL_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta,
                                                 double BMD, double BMRF,
                                                 bool isIncreasing) {
...
////////////////////////////////////////////////////////////////////////
// Function:  double normalEXPONENTIAL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd
...
//
////////////////////////////////////////////////////////////////////////
double normalEXPONENTIAL_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta,
                                                        double BMD, double BMRF,
                                                        bool isIncreasing,
                                                        double TAIL_PROB) {
...
//            double BMRF, bool isIncreasing)
////////////////////////////////////////////////////////////////////////////////////////////////////////
double normalEXPONENTIAL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
                                              double BMRF, bool isIncreasing) {
...
double normalEXPONENTIAL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                                           bool isIncreasing) {
...
double normalEXPONENTIAL_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalEXPONENTIAL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
                                           bool isIncreasing) {
  double min, mid, max;
  min = 0.0;
  max = X.maxCoeff();
  ...
  while (isIncreasing ? t_mean(2, 0) - BMRF < 0
                      : t_mean(2, 0) - BMRF > 0 && niter < 10) {
    niter++;
    max *= 2.0;
...
double normalEXPONENTIAL_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF,
                                           bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
//
////////////////////////////////////////////////////////////////////////
double normalEXPONENTIAL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
                                                  double BMRF,
                                                  bool isIncreasing,
                                                  double TAIL_PROB) {
