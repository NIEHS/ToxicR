												  ...
												  void *data)
{
	...
	case NORMAL_EXP4_UP:
		...
		temp = sdata->isIncreasing ? log(-1.0 * (t + 1.0 - exp(b[2])) / (b[2] - 1.0)) : log((t - 1.0 + exp(b[2])) / (exp(b[2]) - 1.0));
		temp = -1.0 * temp / sdata->BMD;
		break;
	default:
		// default is deg = 5;
		// again don't worry bout increasing/decreasing.
		temp = sdata->isIncreasing ? log(-1.0 * (t + 1.0 - exp(b[2])) / (exp(b[2]) - 1.0)) : log((t - 1.0 + exp(b[2])) / (exp(b[2]) - 1.0));
																	 ...
																	 bool isIncreasing)
{
	...
	case NORMAL_EXP4_DOWN:
	case NORMAL_EXP4_UP:
		temp = isIncreasing ? log(-1.0 * (t + 1.0 - exp(x[2])) / (exp(x[2]) - 1.0)) : log((t - 1.0 + exp(x[2])) / (exp(x[2]) - 1.0));
		temp = -1.0 * temp / BMD;
		break;
	default:
...
double normalEXPONENTIAL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
{
	...
	niter = 0;
	while (fabs(test) > 1e-7 && niter < 200)
	{ // zero in on the BMD
		if (test > 0)
		{
			max = mid;
