...
Eigen::MatrixXd X_cov(Eigen::MatrixXd theta, Eigen::MatrixXd Y,
  ...
  Eigen::MatrixXd rValue =
      data_likelihood.variance(theta); //*Y.col(1).array(); // n*p*(1-p)
 ...
 * Purpose:  This function performs a dichotomous Model Average (MA) for
 *           dichotomous data.  This is done using MCMC estimation.
 * Input:
 *  @dichotomousMA_analysis - Pointer to memory containing information for the
 *                            MA analysis
 *  @dichotomous_analysis   - Pointer to memory containing information about the
 *                            basic type of dichotomous analysis.  Here
 *                            individual model informaiton is discarded.
 * Return Values:
 *  @dichotomousMA_result   - Pointer to the MA result.  Inside the result are
 *                            all of the individual fit information.  The fit
 *                            information is returned using a summary statistic
 *                            format, and is computedusing the proper number
 *                            of burnins.
 ...
 *                  function. Not doing so will result in memory leaks, and may
 *                  be problematic on server environments.
 *
 *                  2) These funcions are overloaded with continuous
 *                  counterparts that use the exact same estimation
...
/* Function: estimate_ma_mcmc
 * Purpose:  This function performs a dichotomous Model Average (MA) for
 * dichotomous data.  This is done using Laplace estimation. Input  :
 *  @dichotomousMA_analysis - Pointer to memory containing information for the
 * MA analysis
 *  @dichotomous_analysis   - Pointer to memory containing information about the
 * basic type of dichotomous analysis.  Here individual model informaiton is
 * discarded. Return Values:
 *  @dichotomousMA_result   - Pointer to the MA result.  Inside the result are
 * all of the individual fit information.  The fit information is returned using
 * a summary statistic format.  This is computed using the methodologies
 * described in Wheeler et al ()2020).
 *
 *
 * SPECIAL NOTES:   1) All memory is assumed allocated by the calling function.
 *                  As a result, any memory should be dealocated by the calling
 *                  function. Not doing so will result in memory leaks!
 *                  This may be very problematic on server environments.
 *                  2) These funcions are overloaded with
 *                  continuous counterparts that use the exact same estimation
 *                  algorithms for continuous data.
 */
 ...
 * This is done using Laplace estimation. Input  :
 *
 *  @dichotomous_analysis   - Pointer to memory containing information about the
 * basic type of dichotomous analysis. Unlike the MA one individual model
 * informaiton needed for the analysis. Return Values:
 *  @dichotomous_model_result   - Pointer to a single model. The fit information
 ...
 * using methodologies described in Wheeler et al (2020).
 *
 *  @bmd_analysis_MCMC          - All of the posterior samples from the MCMC
 * analysis.
 *
 *
 * SPECIAL NOTES:   1) All memory is assumed allocated by the calling function.
...
void estimate_sm_mcmc(dichotomous_analysis *DA, dichotomous_model_result *res,
                      bmd_analysis_MCMC *mcmc, bool do_a_rescale = true);
