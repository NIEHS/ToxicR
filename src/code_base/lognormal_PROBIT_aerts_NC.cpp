...
//////////////////////////////////////////////////////////////////////////
int lognormalPROBIT_aerts_BMD_NC::type_of_profile(contbmd TYPE)
{
...
//                       An equality constraint is used here.
//////////////////////////////////////////////////////////////////////////
int lognormalPROBIT_aerts_BMD_NC::parameter_to_remove(contbmd TYPE) {
...
*           can be performed. This function is used in an optimization that finds the
			...
			unbounded - c
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_absolute(unsigned n,
											  const double *b,
											  double *grad,
											  void   *data){
	...
	temp = gsl_cdf_ugaussian_P(b[0] + b[1] * pow(sdata->BMD, b[3]));
	temp = sdata->BMRF / fabs(temp - gsl_cdf_ugaussian_P(b[0]));
//	temp = (sdata->BMRF + b[0]) / temp;
	returnV 	+= pow(temp - c,2.0);

	return  returnV;
}

std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_absolute_clean(std::vector<double> x,
													...
													bool isIncreasing)
{
	...
	// double temp = 0.0;
	// temp = gsl_cdf_ugaussian_P(x[0] + x[1] * pow(BMD, x[3]));
	// // temp = BMRF / fabs(temp - gsl_cdf_ugaussian_P(x[0]));
	// temp = log(BMRF + exp(x[2] * gsl_cdf_ugaussian_P(x[0]))) / temp;
//	temp = (BMRF + x[0]) / temp;
	double temp = BMRF + exp(x[2] * gsl_cdf_ugaussian_P(x[0]));
	temp = log(temp) / x[2];
	temp = std::max(0.00001, std::min(temp, 0.99999));
	temp = gsl_cdf_ugaussian_Pinv(temp);
...
unbounded - nu
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_reldev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
//		t = 1.0 - sdata->BMRF; // + b[0];
	...
	returnV += pow(d - b[3],2);
	returnV	+= pow(theta(4, 0) - b[4], 2);
	temp = t * gsl_cdf_ugaussian_P(b[0]);
	temp = std::max(0.0, std::min(temp, 0.99999));
	temp = gsl_cdf_ugaussian_Pinv(temp);
	temp =  (b[0] - temp)/ pow(sdata->BMD, b[3]);
	temp = sdata->isIncreasing ? temp : -1.0*temp;
	returnV += pow(temp - bb,2.0);

	return  returnV;
}

std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_reldev_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double t = 0.0;
	...
	double temp = t * exp(x[2] * gsl_cdf_ugaussian_P(x[0]));
	temp = log(temp) / x[2];
	temp = std::max(0.00001, std::min(temp, 0.99999));
	temp = gsl_cdf_ugaussian_Pinv(temp);
	temp =  (temp - x[0])/ pow(BMD, x[3]);
...
unbounded - nu
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_stddev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_stddev_clean(std::vector<double> x,
															  double BMRF, double BMD,
															  bool isIncreasing)
{
	if (!isIncreasing)
...
unbounded - nu
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_point(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_point_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double temp = 0;
...
unbounded - nu
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_extra(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
																bool isIncreasing)
{
...
unbounded - nu
*/
double  lognormalPROBIT_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
												  const double *b,
												  double *grad,
												  void   *data) {
...
std::vector<double> lognormalPROBIT_aerts_BMD_NC::bmd_start_hybrid_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
...
// Functions: lognormalPROBIT_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalPROBIT_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalPROBIT_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//            lognormalPROBIT_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note they are  call the code
//            in  lognormalPROBIT_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalPROBIT_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(2,1); d << 0.0, BMD;
...
double lognormalPROBIT_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
double lognormalPROBIT_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalPROBIT_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
...
double lognormalPROBIT_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalPROBIT_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,,double BPROB)
...
//
////////////////////////////////////////////////////////////////////////
double lognormalPROBIT_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
...
//            in  lognormalPROBIT_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalPROBIT_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	...
	double b = theta(1,0);
	double c = theta(2,0);
	double f = c*gsl_cdf_ugaussian_P(a);
	// double q  = BMRF / exp(theta(2,0) * (1.0 - f));
	double d = theta(3,0);

	double temp = BMRF + exp(f);
	//make sure >0 before taking log
	temp = std::max(1e-6, temp);
	...
	temp = pow((gsl_cdf_ugaussian_Pinv(temp) - a) / b, 1.0/d);

	//v2 -- no Kappa creation
	//double temp = gsl_cdf_ugaussian_Pinv((BMRF + a) / theta(2,0));
	//temp = pow((temp - a) / b, 1.0 / d);

	return temp;

}
double lognormalPROBIT_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = variance(theta,d);
	double t = temp(0,0);
...
double lognormalPROBIT_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalPROBIT_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	// note isIncreasing is ignored as point is specified by the user

	double a = theta(0,0);
	double b = theta(1,0);
	double f = gsl_cdf_ugaussian_P(a);
	double q  = BMRF / exp(theta(2,0) * (1.0 - f));
	double d = theta(3,0);
	...
	//v2 -- no Kappa creation
	//double temp = gsl_cdf_ugaussian_Pinv((BMRF) / theta(2,0));
	//temp = pow((temp - a) / b, 1.0 / d);

	return temp;
}

double lognormalPROBIT_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	double nu = theta(1,0);
	temp = exp(temp.array());
...
//
////////////////////////////////////////////////////////////////////////
double lognormalPROBIT_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,double TAIL_PROB){
	...
	{ // Go up to 2^10 times the maximum tested dose
...
// double lognormalPROBIT_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd theta)
// {
...
// 	mu = mu.unaryExpr([&theta](double xx){return xx - 0.5 * exp(theta(theta.size() - 1, 0));});

// 	if (sufficient_statistics)
// 	{
// 		// this assumes the correct geometric mean and geometric
// 		// standard deviation are specified
// 		returnV = -Y.col(2).array() * Y.col(0).array() + Y.col(2).array() * log(1 / sqrt(2.0 * M_PI)) - (Y.col(2).array() / 2.0) * log(var.array()) - (1 / (2.0 * var.array())) * ((Y.col(2).array() - 1) * pow(Y.col(1).array(), 2) + Y.col(2).array() * pow(Y.col(0).array() - mu.array(), 2));
// 	}
// 	else
// 	{
