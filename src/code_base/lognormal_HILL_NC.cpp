...
*/
double lognormalHILL_BMD_NC::bmd_start_absolute(unsigned n, const double *b,
...
essentially unbounded - nu
*/
double lognormalHILL_BMD_NC::bmd_start_reldev(unsigned n, const double *b,
                                              double *grad, void *data) {
...
std::vector<double>
lognormalHILL_BMD_NC::bmd_start_reldev_clean(std::vector<double> x, double BMRF,
...
essentially unbounded - nu
************************************************/
double lognormalHILL_BMD_NC::bmd_start_stddev(unsigned n, const double *b,
                                              double *grad, void *data) {
...
std::vector<double>
lognormalHILL_BMD_NC::bmd_start_stddev_clean(std::vector<double> x, double BMRF,
                                             double BMD, bool isIncreasing) {
  if (!isIncreasing)
    BMRF *= -1;
  ...
  double temp =
      fabs(x[1] * pow(BMD, x[3]) / (pow(BMD, x[3]) + pow(x[2], x[3]))) / x[0] +
      1;
  temp = log(temp) / BMRF;
  temp = 2.0 * log(temp);
...
essentially unbounded - nu
***********************/
double lognormalHILL_BMD_NC::bmd_start_point(unsigned n, const double *b,
                                             double *grad, void *data) {
  ...
  returnV += pow(n_exp - b[3], 2);
  temp += sdata->BMRF -
          (b[1] * pow(sdata->BMD, b[3])) / (pow(b[2], b[3]) + pow(b[2], b[3]));
  returnV += pow(temp - gamma, 2.0);
...
std::vector<double>
lognormalHILL_BMD_NC::bmd_start_point_clean(std::vector<double> x, double BMRF,
                                            double BMD, bool isIncreasing) {
  double temp = 0;
...
essentially unbounded - nu
*****************************/
double lognormalHILL_BMD_NC::bmd_start_extra(unsigned n, const double *b,
                                             double *grad, void *data) {
...
std::vector<double>
lognormalHILL_BMD_NC::bmd_start_extra_clean(std::vector<double> x, double BMRF,
                                            double BMD, bool isIncreasing) {
...
essentially unbounded - nu
*/
double lognormalHILL_BMD_NC::bmd_start_hybrid_extra(unsigned n, const double *b,
                                                    double *grad, void *data) {
...
/*************************************************
// Functions: lognormalHILL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double
BMRF, bool isIncreasing)
//			  lognormalHILL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta,
double BMRF, bool isIncreasing)
//			  lognormalHILL_BMD_NC::bmd_reldev(Eigen::MatrixXd
theta, double BMRF, bool isIncreasing)
//            lognormalHILL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double
BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note
they are  call the code
//            in  lognormalHILL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
double BMRF, bool isIncreasing)
//
**************************************************/
double lognormalHILL_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta,
...
double lognormalHILL_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD,
                                             double BMRF, bool isIncreasing) {
...
double lognormalHILL_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD,
                                              double BMRF, bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double lognormalHILL_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD,
                                             double BMRF, bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double lognormalHILL_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD,
                                             double BMRF, bool isIncreasing) {
...
/************************************************
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalHILL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd
theta, double BMRF, bool isIncreasing,,double BPROB)
...
////////////////////////////////////////////////////////////////////////
***********************************************/
double lognormalHILL_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta,
                                                    double BMD, double BMRF,
                                                    bool isIncreasing,
                                                    double TAIL_PROB) {
...
//
***********************************************************************************/
double lognormalHILL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF,
                                          bool isIncreasing) {
...
double lognormalHILL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                                       bool isIncreasing) {
...
double lognormalHILL_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                                        bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double lognormalHILL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
                                       bool isIncreasing) {
  // note isIncreasing is ignored as point is specified by the user
...
double lognormalHILL_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF,
                                       bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
////////////////////////////////////////////////////////////////////////
**************************************************************************/
double lognormalHILL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
                                              double BMRF, bool isIncreasing,
                                              double TAIL_PROB) {
