...
double::real negLogLikelihood(const autodiff::ArrayXreal &theta) {//necessary for autodiff 

  // get the mean and variance for each dose group
  autodiff::ArrayXreal  mu = mean(theta); 
  autodiff::ArrayXreal  var = variance(theta); 
  
  autodiff::ArrayXreal returnV = Y.col(0); // make a log likelihood value
  ...
  if (sufficient_statistics){
    ...
    autodiff::ArrayXreal  sqerr = pow(Y.col(0).array()-mu.array(),2); 
    returnV = -(0.5)*log(2*M_PI*var.array())-(1/(2.0*var.array()))*sqerr.array(); 
    return -returnV.sum();
