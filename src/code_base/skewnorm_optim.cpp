...
double skewnorm_pdf(double x, void *params) {
  struct skewnorm_params *p = (struct skewnorm_params *)params;
  double xi = p->xi;
...
double skewnorm_quantile(double q, double xi) {
  ...
  //	FDF.params = &params;
  //
  //	T = gsl_root_fdfsolver_newton; // or steffenson might be faster
  //(fdfsolver_steffenson) 	s = gsl_root_fdfsolver_alloc (T);
