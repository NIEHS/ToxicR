...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_absolute_clean(
...
essentially unbounded - nu
*/
double normalIGAMMA_aerts_BMD_NC::bmd_start_reldev(unsigned n, const double *b,
                                                   double *grad, void *data) {
...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_reldev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double t = 0.0;
...
essentially unbounded - nu
*/
double normalIGAMMA_aerts_BMD_NC::bmd_start_stddev(unsigned n, const double *b,
                                                   double *grad, void *data) {
  ...
  // manually fix the first value to not be (potentially) NA but a, which is
...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_stddev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  if (!isIncreasing)
    BMRF *= 1;
...
essentially unbounded - nu
*/
double normalIGAMMA_aerts_BMD_NC::bmd_start_point(unsigned n, const double *b,
                                                  double *grad, void *data) {
...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_point_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double temp = 0;
...
essentially unbounded - nu
*/
double normalIGAMMA_aerts_BMD_NC::bmd_start_extra(unsigned n, const double *b,
                                                  double *grad, void *data) {
...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
...
std::vector<double> normalIGAMMA_aerts_BMD_NC::bmd_start_hybrid_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing,
    double tail_prob) {
...
// Functions: normalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
// double BMRF, bool isIncreasing)
//			  normalIGAMMA_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd
//theta, double BMRF, bool isIncreasing)
//			  normalIGAMMA_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd
//theta, double BMRF, bool isIncreasing)
//            normalIGAMMA_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double
//            BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note
// they are  call the code
//            in  normalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
//            double BMRF, bool isIncreasing)
//
double normalIGAMMA_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta,
...
double normalIGAMMA_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
...
double normalIGAMMA_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta,
                                                   double BMD, double BMRF,
                                                   bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalIGAMMA_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalIGAMMA_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
...
////////////////////////////////////////////////////////////////////////
// Function:  double normalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd
// theta, double BMRF, bool isIncreasing,,double BPROB) Variables: theta -
// matrix of theta values for the model
...
//
////////////////////////////////////////////////////////////////////////
double normalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta,
                                                         double BMD,
                                                         double BMRF,
                                                         bool isIncreasing,
                                                         double TAIL_PROB) {
...
//            double BMRF, bool isIncreasing)
//
double normalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
                                               double BMRF, bool isIncreasing) {
...
double normalIGAMMA_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = variance(theta, d);
  double t = temp(0, 0);
  //	cout << "STD t,bmd: " << t << ", " <<
  //bmd_absolute(theta,BMRF*pow(t,0.5),isIncreasing) << endl;

  return bmd_absolute(theta, BMRF * pow(t, 0.5), isIncreasing);
}
double normalIGAMMA_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                                             bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalIGAMMA_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  // note isIncreasing is ignored as point is specified by the user
...
double normalIGAMMA_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
//
////////////////////////////////////////////////////////////////////////
double normalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
                                                   double BMRF,
                                                   bool isIncreasing,
                                                   double TAIL_PROB) {
  ...
  if (isfinite(mid)) {
    ...
    // std::cerr << "Non-finite BMD returned: Inverse Gamma-Normal."<<
    // std::endl;
    return std::numeric_limits<double>::infinity();
  }
}

// log likelihood with check for NAs and replace with a (numerical instability
// issue with gammas)
