...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_absolute_clean(
...
essentially unbounded - nu
*/
double normalILOMAX_aerts_BMD_NC::bmd_start_reldev(unsigned n, const double *b,
                                                   double *grad, void *data) {
...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_reldev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double t = 0.0;
...
essentially unbounded - nu
*/
double normalILOMAX_aerts_BMD_NC::bmd_start_stddev(unsigned n, const double *b,
                                                   double *grad, void *data) {
...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_stddev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  if (!isIncreasing)
    BMRF *= 1;
...
essentially unbounded - nu
*/
double normalILOMAX_aerts_BMD_NC::bmd_start_point(unsigned n, const double *b,
                                                  double *grad, void *data) {
...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_point_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
  double temp = 0;
  temp = pow(x[1] / (x[1] + pow(BMD, -1.0 * x[3])), x[4]);
...
essentially unbounded - nu
*/
double normalILOMAX_aerts_BMD_NC::bmd_start_extra(unsigned n, const double *b,
                                                  double *grad, void *data) {
...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing) {
...
std::vector<double> normalILOMAX_aerts_BMD_NC::bmd_start_hybrid_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing,
    double tail_prob) {
...
// Functions: normalILOMAX_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
// double BMRF, bool isIncreasing)
//			  normalILOMAX_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd
//theta, double BMRF, bool isIncreasing)
//			  normalILOMAX_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd
//theta, double BMRF, bool isIncreasing)
//            normalILOMAX_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double
//            BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note
// they are  call the code
//            in  normalILOMAX_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
//            double BMRF, bool isIncreasing)
//
double normalILOMAX_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta,
...
double normalILOMAX_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
...
double normalILOMAX_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta,
                                                   double BMD, double BMRF,
                                                   bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalILOMAX_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalILOMAX_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta,
                                                  double BMD, double BMRF,
                                                  bool isIncreasing) {
...
////////////////////////////////////////////////////////////////////////
// Function:  double normalILOMAX_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd
...
//
////////////////////////////////////////////////////////////////////////
double normalILOMAX_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta,
                                                         double BMD,
                                                         double BMRF,
                                                         bool isIncreasing,
                                                         double TAIL_PROB) {
...
//            double BMRF, bool isIncreasing)
//
double normalILOMAX_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta,
                                               double BMRF, bool isIncreasing) {
...
double normalILOMAX_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = variance(theta, d);
...
double normalILOMAX_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                                             bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalILOMAX_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  // note isIncreasing is ignored as point is specified by the user
...
double normalILOMAX_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF,
                                            bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
//
////////////////////////////////////////////////////////////////////////
double normalILOMAX_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
                                                   double BMRF,
                                                   bool isIncreasing,
                                                   double TAIL_PROB) {
  ...
  if (isfinite(mid)) {
    ...
    // std::cerr << "Non-finite BMD returned: Inverse Lomax-Normal."<<
    // std::endl;
    return std::numeric_limits<double>::infinity();
  }
}
