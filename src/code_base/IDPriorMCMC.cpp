...
double IDPriorMCMC::neg_log_prior(Eigen::MatrixXd theta,
  ...
  for (int i = 0; i < theta.rows(); i++) {
    ...
    case 3:
      ...
      returnV += log(
          gsl_ran_cauchy_pdf(theta(i, 0) - prior_spec(i, 1), prior_spec(i, 2)));
      break;
    case 4:
      returnV -= log(0.5 * M_2_SQRTPI * M_SQRT1_2);
      returnV += log(gsl_ran_gamma_pdf(
          theta(i, 0), pow(prior_spec(i, 1) / prior_spec(i, 2), 2),
          pow(prior_spec(i, 2), 2) / prior_spec(i, 1)));
      break;
    case 5: {
      returnV -= log(0.5 * M_2_SQRTPI * M_SQRT1_2);
      double gam = ((prior_spec(i, 1) - prior_spec(i, 3)) *
                    (prior_spec(i, 4) - prior_spec(i, 1))) /
                       (pow(prior_spec(i, 2), 2)) -
                   3.0;
      double b = ((gam + 2.0) * prior_spec(i, 1) - prior_spec(i, 3) -
                  prior_spec(i, 4)) /
                 gam;
      double alpha = 1.0 + gam * (b - prior_spec(i, 3)) /
                               (prior_spec(i, 4) - prior_spec(i, 3));
      double beta = 1.0 + gam * (prior_spec(i, 4) - b) /
                              (prior_spec(i, 4) - prior_spec(i, 3));
      ...
      returnV -=
          (alpha + beta - 1.0) * log(prior_spec(i, 4) - prior_spec(i, 3));
    } break;
...
Eigen::MatrixXd IDPriorMCMC::log_prior(Eigen::MatrixXd theta) {
  ...
  for (int i = 0; i < theta.rows(); i++) {
    ...
    case 2:
      ...
      // returnV(i,0) = - 0.5*mean*mean / (sd*sd) ;// -log(sd) - log(theta(i,
      // 0)) + pi_const;
      break;
    case 3:
    case 4:
...
// Output: Returns the prior mean for normal and median for the log normal
// distributions
//		   for improper prior it returns the value that is in column 2,
// which
