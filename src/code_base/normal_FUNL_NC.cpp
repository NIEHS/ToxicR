...
*/
double normalFUNL_BMD_NC::bmd_start_absolute(unsigned n, const double *b,
  ...
  temp = exp(-exp(b[5]) * (BMD - b[4]) * (BMD - b[4])) *
         (1 / (1 + exp(-(BMD - b[2]) / b[3])));
  temp -= exp(-exp(b[5]) * (0.0 - b[4]) * (0.0 - b[4])) *
          (1 / (1 + exp(-(0.0 - b[2]) / b[3])));
...
std::vector<double>
normalFUNL_BMD_NC::bmd_start_absolute_clean(std::vector<double> x, double BMRF,
  ...
  temp = exp(-exp(b6) * (BMD - b5) * (BMD - b5)) *
         (1 / (1 + exp(-(BMD - b3) / b4)));
  temp -= exp(-exp(b6) * (0.0 - b5) * (0.0 - b5)) *
          (1 / (1 + exp(-(0.0 - b3) / b4)));
  temp = BMRF / temp;
  x[1] = temp;
  return x;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_reldev(unsigned n, const double *b,
                                           double *grad, void *data) {
  ...
  returnV += pow(b6 - b[5], 2);
  returnV += pow(theta(6, 0) - b[6], 2); // variance parameter
  temp = exp(-b6 * (sdata->BMD - b5) * (sdata->BMD - b5)) *
         (1 / (1 + exp(-(sdata->BMD - b3) / b4)));
  temp -= (1 + t) * exp(-exp(b6) * (0.0 - b5) * (0.0 - b5)) *
          (1 / (1 + exp(-(0.0 - b3) / b4)));
  temp = b[0] * t / temp;
...
std::vector<double>
normalFUNL_BMD_NC::bmd_start_reldev_clean(std::vector<double> b, double BMRF,
                                          double BMD, bool isIncreasing) {
  double t;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_stddev(unsigned n, const double *b,
                                           double *grad, void *data) {
...
std::vector<double>
normalFUNL_BMD_NC::bmd_start_stddev_clean(std::vector<double> x, double BMRF,
                                          double BMD, bool isIncreasing) {
  if (!isIncreasing)
    BMRF *= 1;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_point(unsigned n, const double *b,
                                          double *grad, void *data) {
  ...
  returnV += pow(theta(6, 0) - b[6], 2); // variance parameter
  temp = (sdata->BMRF) /
         exp(-exp(b[5]) * (sdata->BMD - b[4]) * (sdata->BMD - b[4])) *
...
std::vector<double>
normalFUNL_BMD_NC::bmd_start_point_clean(std::vector<double> b, double BMRF,
                                         double BMD, bool isIncreasing) {
  double temp = 0;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_extra(unsigned n, const double *b,
                                          double *grad, void *data) {
...
std::vector<double>
normalFUNL_BMD_NC::bmd_start_extra_clean(std::vector<double> x, double BMRF,
                                         double BMD, bool isIncreasing) {
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_hybrid_extra(unsigned n, const double *b,
                                                 double *grad, void *data) {
...
// Functions: normalFUNL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double
// BMRF, bool isIncreasing)
//			  normalFUNL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double
//BMRF, bool isIncreasing) 			  normalFUNL_BMD_NC::bmd_reldev(Eigen::MatrixXd theta,
//double BMRF, bool isIncreasing)
//            normalFUNL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
//            bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note
// they are  call the code
//            in  normalFUNL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double
//            BMRF, bool isIncreasing)
//
double normalFUNL_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,
                                             double BMRF, bool isIncreasing) {
...
double normalFUNL_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD,
                                          double BMRF, bool isIncreasing) {
...
double normalFUNL_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD,
                                           double BMRF, bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalFUNL_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD,
                                          double BMRF, bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalFUNL_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD,
                                          double BMRF, bool isIncreasing) {
...
////////////////////////////////////////////////////////////////////////
// Function:  double normalFUNL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta,
...
//
////////////////////////////////////////////////////////////////////////
double normalFUNL_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta,
...
//            BMRF, bool isIncreasing)
//
double normalFUNL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF,
                                       bool isIncreasing) {
...
double normalFUNL_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF,
                                    bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = variance(theta, d);
...
double normalFUNL_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF,
                                     bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
double normalFUNL_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF,
                                    bool isIncreasing) {
...
double normalFUNL_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF,
                                    bool isIncreasing) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
  Eigen::MatrixXd temp = mean(theta, d);
...
//
////////////////////////////////////////////////////////////////////////
double normalFUNL_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF,
                                           bool isIncreasing,
                                           double TAIL_PROB) {
