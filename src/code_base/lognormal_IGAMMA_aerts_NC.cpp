...
int lognormalIGAMMA_aerts_BMD_NC::type_of_profile(contbmd TYPE)
{
...
//////////////////////////////////////////////////////////////////////////
int lognormalIGAMMA_aerts_BMD_NC::parameter_to_remove(contbmd TYPE) {
...
*           can be performed. This function is used in an optimization that finds the
			...
			unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_absolute(unsigned n,
											  const double *b,
											  double *grad,
											  void   *data){
	...
	temp = 1.0 - gsl_cdf_gamma_P(pow(sdata->BMD, -1.0 * b[3]), b[4], 1.0 / b[1]) + 1e-6;
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_absolute_clean(std::vector<double> x,
													...
													bool isIncreasing)
{
	...
	double temp = 0.0;
	temp = 1.0 - gsl_cdf_gamma_P(pow(BMD, -1.0 * x[3]), x[4], 1.0 / x[1]) + 1e-9;
...
unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_reldev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_reldev_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double t = 0.0;
...
unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_stddev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
	...
	// manually fix the first value to not be (potentially) NA but a, which is what it should evaluate to
//	mu(0,0) = b[0];
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_stddev_clean(std::vector<double> x,
															  double BMRF, double BMD,
															  bool isIncreasing)
{
	if (!isIncreasing)
	...
	mu = exp(mu.array());
	// manually fix the first value to not be (potentially) NA but a, which is what it should evaluate to
	// mu(0,0) = exp(log(theta_2(0,0)) + 0.5 * exp(theta_2(5,0)));
	mu(0,0) = exp(theta_2(0,0));
...
unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_point(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
	...
	returnV += pow(xi - b[4],2);
	returnV	+= pow(theta(5, 0) - b[5], 2);
	temp = 1.0 - gsl_cdf_gamma_P(pow(sdata->BMD, -1.0 * b[3]), b[4], 1.0 / b[1]);
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_point_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double temp = 0;
	temp = 1.0 - gsl_cdf_gamma_P(pow(BMD, -1.0 * x[3]), x[4], 1.0 / x[1]);
...
unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_extra(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
																bool isIncreasing)
{
...
unbounded - nu
*/
double  lognormalIGAMMA_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
												  const double *b,
												  double *grad,
												  void   *data) {
...
std::vector<double> lognormalIGAMMA_aerts_BMD_NC::bmd_start_hybrid_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
...
// Functions: lognormalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalIGAMMA_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalIGAMMA_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//            lognormalIGAMMA_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note they are  call the code
//            in  lognormalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalIGAMMA_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(2,1); d << 0.0, BMD;
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,,double BPROB)
...
//
////////////////////////////////////////////////////////////////////////
double lognormalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
...
//            in  lognormalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalIGAMMA_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	...
	double temp = gsl_cdf_gamma_Pinv(1.0 - q, xi, 1.0);
	temp = pow(b / temp, 1.0 / d);
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = variance(theta,d);
	double t = temp(0,0);
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp(0,0) = exp(theta(0,0));// + 0.5 * exp(theta(5,0)));
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	// note isIncreasing is ignored as point is specified by the user
...
double lognormalIGAMMA_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	double nu = theta(1,0);
...
//
////////////////////////////////////////////////////////////////////////
double lognormalIGAMMA_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,double TAIL_PROB){
...
// // log likelihood with check for NAs and replace with a (numerical instability issue with gammas)
// double lognormalIGAMMA_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd theta)
// {
...
// 	// Only for Inv-gamma and gamma, need to check for NaNs and replace
// 	mu = mu.unaryExpr([&theta](double xx){
// 		// only does anything if NaN (NaN != NaN)
// 		if(xx != xx){
