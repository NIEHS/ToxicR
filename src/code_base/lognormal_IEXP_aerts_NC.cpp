...
int lognormalIEXP_aerts_BMD_NC::type_of_profile(contbmd TYPE)
{
...
//////////////////////////////////////////////////////////////////////////
int lognormalIEXP_aerts_BMD_NC::parameter_to_remove(contbmd TYPE) {
...
*           can be performed. This function is used in an optimization that finds the
			...
			unbounded - c
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_absolute(unsigned n,
											  const double *b,
											  double *grad,
											  void   *data){
	...
	temp = exp(-1.0 * b[1] * pow(sdata->BMD, -1.0*b[3]));
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_absolute_clean(std::vector<double> x,
													...
													bool isIncreasing)
{
	...
	double temp = 0.0;
	temp = exp(-1.0 * x[1] * pow(BMD, -1.0*x[3]));
	//make sure it is within realm of possibility (prevent overflow)
	temp = std::max(temp, 1e-3 / x[0]);
...
unbounded - nu
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_reldev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_reldev_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double t = 0.0;
...
unbounded - nu
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_stddev(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_stddev_clean(std::vector<double> x,
															  double BMRF, double BMD,
															  bool isIncreasing)
{
	if (!isIncreasing)
...
unbounded - nu
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_point(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_point_clean(std::vector<double> x,
	double BMRF, double BMD,
	bool isIncreasing)
{
	double temp = 0;
...
unbounded - nu
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_extra(unsigned n,
											const double *b,
											double *grad,
											void   *data) {
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
																bool isIncreasing)
{
...
unbounded - nu
*/
double  lognormalIEXP_aerts_BMD_NC::bmd_start_hybrid_extra(unsigned n,
												  const double *b,
												  double *grad,
												  void   *data) {
...
std::vector<double> lognormalIEXP_aerts_BMD_NC::bmd_start_hybrid_extra_clean(std::vector<double> x,
																double BMRF, double BMD,
...
// Functions: lognormalIEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalIEXP_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//			  lognormalIEXP_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//            lognormalIEXP_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
// Purpose :  return the BMD given the parameter values theta and the BMRF. Note they are  call the code
//            in  lognormalIEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalIEXP_aerts_BMD_NC::bmd_absolute_bound(Eigen::MatrixXd theta, double BMD,double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(2,1); d << 0.0, BMD;
...
double lognormalIEXP_aerts_BMD_NC::bmd_stdev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
double lognormalIEXP_aerts_BMD_NC::bmd_reldev_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalIEXP_aerts_BMD_NC::bmd_extra_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalIEXP_aerts_BMD_NC::bmd_point_bound(Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing){
...
////////////////////////////////////////////////////////////////////////
// Function:  double lognormalIEXP_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,,double BPROB)
...
//
////////////////////////////////////////////////////////////////////////
double lognormalIEXP_aerts_BMD_NC::bmd_hybrid_extra_bound(Eigen::MatrixXd theta, double BMD,
...
//            in  lognormalIEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing)
//
double lognormalIEXP_aerts_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	...
	double temp = (-1.0 * b) / (log(q));
	temp = pow(temp, 1.0 / d);

	return temp;

}
double lognormalIEXP_aerts_BMD_NC::bmd_stdev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = variance(theta,d);
	double t = temp(0,0);
...
double lognormalIEXP_aerts_BMD_NC::bmd_reldev(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	temp = exp(temp.array());
...
double lognormalIEXP_aerts_BMD_NC::bmd_point(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	// note isIncreasing is ignored as point is specified by the user
...
double lognormalIEXP_aerts_BMD_NC::bmd_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing){
	Eigen::MatrixXd d(1,1); d(0,0) = 0.0;
	Eigen::MatrixXd temp = mean(theta,d);
	double nu = theta(1,0);
...
//
////////////////////////////////////////////////////////////////////////
double lognormalIEXP_aerts_BMD_NC::bmd_hybrid_extra(Eigen::MatrixXd theta, double BMRF, bool isIncreasing,double TAIL_PROB){
...
// double lognormalIEXP_aerts_BMD_NC::negLogLikelihood(Eigen::MatrixXd theta)
// {
